...
class WaveSliderState extends State<WaveSlider> {
  ...
  void initState() {
    ...
    heightSlider = (widget.heightWaveSlider < 50) ? 60 : widget.heightWaveSlider;
...
class Bar extends StatelessWidget {
  ...
  const Bar(
      {Key? key, required this.position, required this.width, required this.callback, required this.callbackEnd, this.colorBG,required this.isLeft})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: GestureDetector(
        ...
        onHorizontalDragEnd: callbackEnd,
        child: Container(
          decoration: BoxDecoration(borderRadius: isLeft? BorderRadius.only(topLeft: Radius.circular(10),bottomLeft: Radius.circular(10)):BorderRadius.only(topRight: Radius.circular(10),bottomRight: Radius.circular(10)),color: colorBG ?? Colors.red,),
          height: 50,
          width: width,
          child:  Padding(
            padding: const EdgeInsets.all(5.0),
            child: Icon(isLeft?Icons.arrow_back_ios:Icons.arrow_forward_ios, size: 18, color: Colors.white,fontWeight: FontWeight.bold,),
          ),
        ),
      ),
    );
