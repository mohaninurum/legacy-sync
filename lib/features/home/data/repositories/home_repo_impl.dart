...
import 'package:legacy_sync/features/home/data/model/remove_friend_response_model.dart';
...
class HomeRepoImpl extends HomeRepo {
  ...
  ResultFuture<FriendsListModel> getFriendsList({required int userId}) async {
    try {
      final res = await _apiServices.getGetApiResponse(
        "${ApiURL.GET_FRIEND_LIST}$userId",
      );
      return res.fold(
        (error) => Left(error),
        (data) => Right(FriendsListModel.fromJson(data)),
      );
  ...
  @override
  ResultFuture<AddFriendResponseModel> addFriend({
    ...
    try {
      ...
      return res.fold(
        (error) => Left(error),
        (data) => Right(AddFriendResponseModel.fromJson(data)),
      );
    } on AppException catch (e) {
      return Left(e);
    }
  ...
  @override
  ResultFuture<RemoveFriendResponseModel> removeFriend({
    required Map<String, dynamic> body,
  }) async {
    try {
      final res = await _apiServices.getDeleteApiResponse(ApiURL.REMOVE_FRIEND, body);
      return res.fold(
        (error) => Left(error),
        (data) => Right(RemoveFriendResponseModel.fromJson(data)),
      );
    } on AppException catch (e) {
      return Left(e);
    }
  ...
  ResultFuture<HomeJourneyCardModel> getHomeModule({required int userID}) async {
    try {
      final res = await _apiServices.getGetApiResponse(
        "${ApiURL.GET_LEGACY_HOME_MODULE}$userID",
      );
      return res.fold(
        (error) => Left(error),
        (data) => Right(HomeJourneyCardModel.fromJson(data)),
      );
    } on AppException catch (e) {
      return Left(e);
    }
